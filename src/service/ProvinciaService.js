...
export const getProvincias = async (state) => {
    await axios.get(url).then((resp) => state(resp.data.result));
};

export const createProvince = async (provincia) => {
    return await axios.post(url, provincia);
};

export const updateProvincia = async (provincia) => {
    await axios.put(url, provincia);
};

export const deleteProvincia = async (id, setActive) => {
    await axios
        .delete(url + id)
